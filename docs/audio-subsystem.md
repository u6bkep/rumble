...
2. When capture is active, the **TX pipeline's `suppress`** (typically from denoise/VAD) gates each individual frame: suppressed frames aren't encoded/sent, and the active→suppressed edge emits an `EndOfStream`.
