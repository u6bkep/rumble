...
- Capture callback per frame: discard first 3 frames (stale hardware ringbuffer); measure pre-pipeline RMS; run the TX `AudioPipeline`; measure post-pipeline RMS; if `suppress` (ignored in PTT mode — see Transmission Gating) → emit `EndOfStream` (on the active→suppressed edge) and return; else encode and send `EncodedFrame`.
