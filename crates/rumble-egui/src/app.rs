...
use rumble_client::{
    AudioSettings, Command, ConnectConfig, ConnectionState, PipelineConfig, ProcessorRegistry, SfxKind, VoiceMode,
...
pub struct RumbleApp {
    ...
    #[cfg(unix)]
    _rpc_server: Option<rumble_client::rpc::RpcServer>,
}
...
impl RumbleApp {
    ...
    pub fn new(ctx: egui::Context, runtime_handle: tokio::runtime::Handle, args: Args) -> Self {
        ...
        let rpc_server = if args.rpc_server || args.rpc_socket.is_some() {
            let socket_path = args
                ...
                .unwrap_or_else(rumble_client::rpc::default_socket_path);
    ...
    fn render_settings_connection(&mut self, ui: &mut egui::Ui, state: &rumble_client::State) {
    ...
    fn render_settings_devices(&mut self, ui: &mut egui::Ui, state: &rumble_client::State) {
    ...
    fn render_settings_voice(&mut self, ui: &mut egui::Ui, state: &rumble_client::State) {
    ...
    fn render_settings_processing(&mut self, ui: &mut egui::Ui, state: &rumble_client::State) {
    ...
    pub fn render(&mut self, ctx: &egui::Context) {
                                ...
                                .show(ui, |ui| {
                                    ...
                                    for msg in &state.chat_messages {
                                        ...
                                        if msg.is_local {
                                            ...
                                            match &msg.kind {
                                                rumble_client::events::ChatMessageKind::DirectMessage { .. } => {
                                                    let prefix = format!(
                                                        "{}[DM] {}: {}",
                                                ...
                                                rumble_client::events::ChatMessageKind::Tree => {
                                                    let prefix = format!(
                                                        "{}[Tree] {}: {}",
                                                        timestamp_prefix, msg.sender, msg.text
                                                    );
                                                ...
                                                rumble_client::events::ChatMessageKind::Room => {
                                                    ui.label(format!(
                                                        "{}{}: {}",
                                                        timestamp_prefix, msg.sender, msg.text
                                                    ));
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if state.connection.is_connected() && state.rooms.is_empty() {
                ui.horizontal(|ui| {
                    ...
                    if ui.button("Join Root").clicked() {
                        self.join_room(rumble_protocol::ROOT_ROOM_UUID);
                    }
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                let mut pending_acl_edit: Option<(Uuid, String, bool, Vec<rumble_protocol::proto::RoomAclEntry>)> =
                    None;
                ...
                let (_response, actions) = TreeView::<TreeNodeId>::new(egui::Id::new("room_tree"))
                    ...
                    .show(ui, |builder| {
                        ...
                        fn render_room(
                            ...
                            pending_acl_edit: &mut Option<(
                                Uuid,
                                String,
                                bool,
                                Vec<rumble_protocol::proto::RoomAclEntry>,
                            )>,
                            ...
                            let is_root = room_id == rumble_protocol::ROOT_ROOM_UUID;
                        ...
                        fn render_user(
                            room_id: Uuid,
                            user: &rumble_protocol::proto::User,
                            state: &rumble_client::State,
                            pending_commands: &mut Vec<Command>,
                            pending_kick: &mut Option<(u64, String)>,
                            pending_ban: &mut Option<(u64, String)>,
            ...
            let modal = Modal::new(egui::Id::new("room_acl_modal")).show(ctx, |ui| {
                ...
                egui::Sides::new().show(
                    ...
                    |ui| {
                        ui.style_mut().spacing.item_spacing.x = 8.0;
                        if ui.button("Save").clicked() {
                            let entries: Vec<rumble_protocol::proto::RoomAclEntry> = self
                                .room_acl_modal
                                .entries
                                .iter()
