...
/// Serializable audio settings (mirrors rumble_client::AudioSettings encoder settings).
///
...
// Hotkey types live in `rumble_desktop_shell::hotkeys`. Re-exported
// from `lib.rs` for backwards compatibility with existing call sites.
pub use rumble_desktop_shell::{HotkeyBinding, HotkeyModifiers, KeyboardSettings};
