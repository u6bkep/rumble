//! "Mumble classic" paradigm — menubar + toolbar + nested tree + chat +
...
fn sep(ui: &mut Ui) {
    ...
    let (rect, _) = ui.allocate_exact_size(egui::Vec2::new(1.0, 18.0), egui::Sense::hover());
...
fn side_header(ui: &mut Ui) {
    SurfaceFrame::new(SurfaceKind::Pane)
    ...
    SurfaceFrame::new(SurfaceKind::Statusbar)
        ...
        .show(ui, |ui| {
            ui.horizontal(|ui| {
                ...
                let server_addr = match &state.connection {
                ...
                ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                    ui.label(
                        RichText::new(format!("· {server_addr}"))
                            .color(tokens.text_muted)
