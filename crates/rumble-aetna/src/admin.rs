...
fn render_members_section(
    ...
    let members_block = if chips.is_empty() {
        ...
        // row by leaning on stack-as-flow. aetna doesn't have a
...
pub fn handle_event(
    ...
    // Accordion toggle.
    if aetna_core::widgets::accordion::apply_event(&mut state.expanded_group, event, ACCORDION_KEY, |raw| {
        Some(raw.to_string())
    }) {
...
fn handle_search_event(state: &mut AdminState, event: &UiEvent, selection: &mut Selection) -> Option<AdminOutcome> {
    if aetna_core::widgets::text_input::apply_event(&mut state.search_query, selection, KEY_SEARCH, event) {
...
fn handle_add_user_query_event(
    ...
    if aetna_core::widgets::text_input::apply_event(&mut popover.query, selection, &key, event) {
        return Some(AdminOutcome::Handled);
    }
