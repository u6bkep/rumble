...
//! composites them through damascene's `surface()` widget into a
...
/// Decode the first frame of `path` into an damascene [`Image`] sized
...
fn lightbox_body(active: &ActiveVideo) -> El {
    ...
    // The 33ms `redraw_within` keeps damascene's loop ticking while
    ...
    // here too (damascene translates Space-with-focus to Activate
