...
//! CPU [`damascene_core::Image`] frames. To play one through damascene's
//! `surface()` widget we keep a single `wgpu::Texture` per active
//! entry and write the current frame's pixels in-place when the
//! playhead advances. Damascene only samples it — no per-frame upload
//! through damascene's image content-hash cache.
//!
...
pub struct AnimatedGpu {
    ...
    /// frame advance; damascene composites by sampling.
    texture: Arc<wgpu::Texture>,
    /// Damascene wrapper around `texture`, handed to `surface(...)` in the
...
impl AnimatedGpu {
    ...
    pub fn allocate(device: &wgpu::Device, frame: &Image) -> Self {
        ...
        let texture = Arc::new(device.create_texture(&wgpu::TextureDescriptor {
            label: Some("rumble-damascene::animated_preview"),
            size: wgpu::Extent3d {
                width: size.0,
                height: size.1,
